...
let concat (x: uint64) (y: uint64) = uint64 ((string x) + (string y))

let rec validate (target: uint64) (res: uint64) (vals: uint64 list) =
    match vals with
    | [v] -> target = (res + v) || target = (res * v) || target = (concat res v)
    | v::vs -> validate target (res + v) vs || validate target (res * v) vs || validate target (concat res v) vs
